...
class ParentA:
    def __init__(self):
        super().__init__()
        '''
        슈퍼는 MRO '순서상' 의 부모를 호출한다.
        child 클래스에서 시작되므로, 현재 상황의 MRO 순서는 Child -> ParentA -> ParentB (-> Object)  이다.
        때문에 ParentA 안의 Super는 ParentA 의 부모를 찾는 함수가 아닌,
        ParentA 다음의 클래스인 ParentB 의 __init__ 메서드가 호출된다.
        '''
